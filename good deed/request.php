...
<head>
    ...
    <script>
        function al(){
            alert("Request was alloted successfully");
...
<body>
            ...
            <p style="text-align:right;padding-top:20px;"><a href="donor.php" >Home</a>&emsp;&emsp;&emsp;&emsp;<a href="profile.php"  >Profile</a>
  ...
  $sq= "SELECT * FROM donation WHERE donorid=$data and  requested='yes' and approved='no'";
  $res = $conn->query($sq);
  ...
  if ($res->num_rows > 0) {
    ...
    <?php
      ...
      while ($r = $res->fetch_assoc()) {
        ...
        <td><form  method="post" >
            <input type="hidden" id="r" name="r" value="<?php echo $r['reqid']; ?>" >
            <input type="hidden" id="d" name="d" value="<?php echo $r['ddate']; ?>" >
            <input type="hidden" id="t" name="t" value="<?php echo $r['dtime']; ?>" >
            <input type="submit" id="a" name="a" value="Approve"> or <input type="submit" id="de" name="de" value="Delete">
        </form>
        </td>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql="UPDATE donation SET approved='yes' WHERE  donorid=$data and ddate=$date and dtime=$time ";
    $result=$conn->query($sql);
    if($result==true){
        ?><script>al();</script><?php
    }
    else{
        echo "failed";
    }
  ...
  elseif(isset($_POST['de'])){
    ...
    $s="UPDATE donation SET requested='no', reqid='none' WHERE donorid=$data and ddate=$date and dtime=$time ";
    $re=$conn->query($s);
    if($re==true){
