...
<body>
  ...
  $sq= "SELECT * FROM donation WHERE reqid=$data and  requested='yes' and donated='no'";
  $res = $conn->query($sq);
  ...
  if ($res->num_rows > 0) {
    ...
    <tr>
        ...
        <th>Approved</th><th>Donation recieved</th><th>Delete Request</th>
    </tr>
    <?php
      ...
      while ($r = $res->fetch_assoc()) {
        ...
        <td><form  method="post" >
            ...
            <input type="hidden" id="t" name="t" value="<?php echo $r['dtime']; ?>" >
             <input type="submit" id="dr" name="dr" value="Recieved?"><td><input type="submit" id="de" name="de" value="Delete?"></td>
        </form>
        </td>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql="UPDATE donation SET donated='yes' WHERE  donorid=$do and ddate=$date and dtime=$time ";
    $result=$conn->query($sql);
    if($result==true){

        $ss="SELECT * FROM donor WHERE phone=$do ";
        $rs=$conn->query($ss);
        while($r = $rs->fetch_assoc()){
            $n=$r['point'];
        }
        $new=$n+10;
        $su="UPDATE donor SET point=$new WHERE phone=$do ";
        $ru=$conn->query($su);
        if($ru==true){
  ...
  elseif(isset($_POST['de'])){
    ...
    $s="UPDATE donation SET requested='no', reqid='none',approved='no' WHERE donorid=$do and ddate=$date and dtime=$time ";
    $re=$conn->query($s);
    if($re==true){
